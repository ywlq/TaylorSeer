...
#             img_mlp = self.act_mlp(self.proj_mlp(norm_img))
#             txt_mlp = self.act_mlp(self.proj_mlp(norm_txt))

#             mlp_hidden_states = torch.cat([img_mlp, txt_mlp], dim=1)
...
#     output = single_cache_add(total_seer, x, gate, proj_out_weight, proj_out_bias)
...
#     proj_out_result = F.linear(total_seer, proj_out_weight, proj_out_bias)
#     x = x + gate * proj_out_result
#     return x
...
#             hidden_states, encoder_hidden_states = double_cache_step(
#                 hidden_states, encoder_hidden_states,
...
#                       txt_attn_dict:dict,
#                       txt_mlp_dict:dict, 
#                       img_mod1:torch.Tensor, img_mod2:torch.Tensor,
#                       txt_mod1:torch.Tensor, txt_mod2:torch.Tensor, 
#                       distance:int):
...
#                      img_mod1:torch.Tensor, img_mod2:torch.Tensor,
#                      txt_mod1:torch.Tensor, txt_mod2:torch.Tensor):
#     img = img + img_mod1 * img_attn_seer + img_mod2 * img_mlp_seer
#     txt = txt + txt_mod1 * txt_attn_seer + txt_mod2 * txt_mlp_seer
#     return img, txt  
...
class HunyuanVideoTransformer3DModelPacked(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def initialize_taylorseer(self, enable_taylorseer=True, num_steps=25):
