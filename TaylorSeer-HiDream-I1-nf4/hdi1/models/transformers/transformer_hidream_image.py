...
class HiDreamImageSingleTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # print(current['stream'],current['layer'])
        ...
        if current['type'] == 'full':
            ...
            image_tokens = single_cache_step(image_tokens,
                                             cache_dic['cache'][-1]['single_stream'][current['layer']]['img_attn'],
                                             cache_dic['cache'][-1]['single_stream'][current['layer']]['img_mlp'],
                                             gate_msa_i, gate_mlp_i,
                                             distance=current['step'] - current['activated_steps'][-1])
        
        return image_tokens
        
@torch.compile
def single_cache_step(img:torch.Tensor, 
                      img_attn_dict:dict,
                      img_mlp_dict:dict,
    ...
    img = single_cache_add(img_attn_seer, img_mlp_seer, img, img_mod1_gate, img_mod2_gate)
    return img

def single_cache_add(img_attn_seer:torch.Tensor, img_mlp_seer:torch.Tensor, img:torch.Tensor, img_mod1_gate, img_mod2_gate):
    
    img = img + img_attn_seer * img_mod1_gate + img_mlp_seer * img_mod2_gate
...
class HiDreamImageTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if current['type'] == 'full':
            ...
            shift_msa_t, scale_msa_t, gate_msa_t, shift_mlp_t, scale_mlp_t, gate_mlp_t = \
                self.adaLN_modulation(adaln_input)[:,None].chunk(12, dim=-1)
            image_tokens, text_tokens = double_cache_step(image_tokens, text_tokens,
...
def double_cache_add(img_attn_seer:torch.Tensor, img_mlp_seer:torch.Tensor,
                     ...
                     txt_mod1_gate, txt_mod2_gate):
    img = img + img_attn_seer * img_mod1_gate + img_mlp_seer * img_mod2_gate
    txt = txt + txt_attn_seer * txt_mod1_gate + txt_mlp_seer * txt_mod2_gate
    return img, txt  
...
class HiDreamImageTransformer2DModel(
    ...
    def forward(
        ...
        **kwargs 
        ...
        # print(current['step'], current['type'])
